...
c** If fitting to band constants for this state ....
     ...
     1                                      NQC(I,IISTP,ISTATE)= NBCMX
                     ENDDO
                  WRITE(6,6082) I,(NBC(I,IISTP,ISTATE),IISTP= 1,NISTP)
     ...
     1                                                          NISTP)
                          IF(NQC(I,ISOT,ISTATE).GT.0) 
     1                    WRITE(20,687) (NQC(I,ISOT,ISTATE),ISOT= 1,
     1                                                          NISTP)
                          ENDDO
                  ENDIF
              GOTO 90
...
c** For Aziz'ian HFD-C potential: print header and derive dimensionless
c     \alpha^* and  A^*  for us in vgen & vgenp
              A1= BETA(0,ISTATE)
              A2= BETA(1,ISTATE)
              A3= BETA(2,ISTATE)
              DX= 1.d0
...
c-----------------------------------------------------------------------
          IF(NPASS.GT.1) WRITE(20,672) NTApr, NTBpr,
