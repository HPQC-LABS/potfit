...
c
     ...
     7  NBANDS(NISTPMX),YPR(NISTPMX,NSTATEMX,8,6,NPARMX)
c
...
c-----------------------------------------------------------------------
     ...
     1                                             VLIM(ISTATE),RR,VV)
      ...
      IF((PSEL(ISTATE).GE.2).AND.(PSEL(ISTATE).LE.5)) THEN
c-----------------------------------------------------------------------
c** For MLR, DELR or HPP or TT potential read number of terms NCMM in 
...
c
c    IDF specifies damping s.th.  Dm(r)/r^m --> r^{IDF/2} as r->0.
...
c  the A^1\Sigma_u^+ state, x=-1  for the 1^3\Sigma_g^+ state & x=-2 for
...
c=======================================================================
      READ(5,*) APSE(ISTATE), Nbeta(ISTATE), nPB(ISTATE), nQB(ISTATE),
     1                                                     RREF(ISTATE)
c=======================================================================
...
c** For APSE .le.0  the MLR exponent  \beta(yp)  is  'conventional'
...
c=======================================================================
        ...
        ELSE
          ...
          DO  I= 1,npow
c-----------------------------------------------------------------------
c** For APSE > 0  exponent is a natural spline function with values BETA
...
c=======================================================================
     ...
     1              .AND.(PSEL(ISTATE).EQ.1)) qAD(ISTATE)= pAD(ISTATE)
      IF(NUA(ISTATE).GE.0) THEN
...
c-----------------------------------------------------------------------
  ...
  606 FORMAT(/' For state ',A2,' represent level energies by independent
     1 band constant for each'/40x,'vibrational levels of each isotologu
     2e'/3x,'No. band constants'/6x,'v','   isotop:  #',I1,
     3   11('   #',I1:) )
...
c-----------------------------------------------------------------------
     ...
     2 DMPP(MMAX),bTT(-2:2),cDS(-4:0),bDS(-4:0), PVSR,BTT,ATT
...
c-----------------------------------------------------------------------
     ...
     1                                                 PU(IPV),PS(IPV)
                ...
                ENDIF
              ENDIF    
  667 FORMAT(1Pd20.12,0PI3,9x,'% ',A2,I2,'  IFX',A2,I2)
  668 FORMAT(1Pd20.12,d20.12,0PI3)
  669 FORMAT(1Pd20.12,0PI3,9x,'% ',A4,I2,'  IFX',A4,I2)
  ...
  674 FORMAT(1Pd20.12,0PI3,9x,'% ',a5,'  IFX',A5)
...
c ... for Aubert-Frecon 2x2 treatment of {C3,C6,C8} for Li2 A-state
...
c=======================================================================
c-----------------------------------------------------------------------
      INTEGER IISTP, IV, I, NPTOT,NB1,COUNT1,ISTATE
      REAL*8 PV(NPARMX), PU(NPARMX), PD(NPARMX), CM(NPARMX,NPARMX), 
     1  PT(NPARMX), Bunc, CALC
c** First - set up fake R(0) datum
      ...
      DO ISTATE= 1, NSTATES
          ...
          DO  IISTP= 1,NISTP
              ISTP(NB1)= IISTP
              WRITE(7,600) IISTP, SLABL(ISTATE)
c** for each vibrational level of each isotopologue ....
              DO  IV= VMIN(ISTATE,IISTP), VMAX(ISTATE,IISTP)
                  VP(NB1)= IV
                  VPP(NB1)= IV
c** First get partial derivatives from Bv as half of an R(0) energy
...
c ... set up column vector for uncertainty calculation
                  DO  I= 1,NPTOT
                      PT(I)= 0.5*PD(I)*PU(I)
                      ENDDO
                  CALL MMCALC(1,NPTOT,PT,CM,Bunc)
c   
                  WRITE(7,602)  IV,CALC/2,Bunc
                  ENDDO
              ENDDO
          ENDDO
      RETURN
  600 FORMAT(/'   Predict Bv uncertainties for isotopologue-',I1,' in st
     1ate ',A2)
  602 FORMAT(' For  v=',I3,'  Bv approx R(0)/2 =', F11.8,'  unc(Bv)=',
     1                                                        1PD12.6)
      END
