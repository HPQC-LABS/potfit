...
c
      REAL*8 Rlr(NPNTMX,NSTATEMX),Vlr(NPNTMX,NSTATEMX),
     1                                            Bsr(NPNTMX,NSTATEMX)
...
c** If HAVE fluorescence series or fitted term values ... first fix ALL 
c  potential parameters and fit to determine estimates of the series 
c  origins and/or term values, and only THEN free potential parameters 
c  too.  First, save read-in values of  'IF(fix)' parameters
          DO  I= 1,TOTPOTPAR
              SIFXPV(I)= IFXPV(I)
              IFXPV(I)= 1
              ENDDO
